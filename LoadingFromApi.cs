...
{
    const string EndpointUri = "<CosmosDB Endpoint URI>";
    const string PrimaryKey = "< CosmosDB Primary Key>";
    ...
    private async Task SeedDatabase(Container container)
    {
        ...
        {
            var postForStorage = blogPost with
            {
                Id = Guid.CreateVersion7()
            };
            var result = await container.CreateItemAsync(postForStorage); 
